...
MolanDev Cloud 基于 **MolanDev Framework** 构建,采用现代化的分层架构设计,支持单体和微服务两种部署模式。
...
MolanDev Cloud 的核心特性是支持**双模部署**,一套代码可以在单体和微服务之间自由切换。
...
```java
// 1. API 定义 (molandev-cloud-api-sys)
@FeignClient(name = "system-service", contextId = "user")
public interface UserApi {
    @GetMapping("/user/get")
    UserVO getUser(@RequestParam("id") Long id);
}

// 2. 服务实现 (molandev-cloud-system-service)
@Service
...
// 3. 其他服务调用 (molandev-cloud-file-service)
@Service
...
| 服务 | 职责 | 核心功能 |
|------|------|----------|
| **System Service** | 系统管理 | 用户、角色、菜单、字典、参数、日志 |
| **File Service** | 文件管理 | 文件上传下载、存储管理、图片处理 |
| **Msg Service** | 消息服务 | 站内信、邮件、短信、消息模板 |
| **Task Service** | 任务调度 | 定时任务、任务调度、执行日志 |
| **Gateway** | 网关路由 | 路由转发、认证鉴权、限流熔断 |

### 公共模块设计

**molandev-cloud-common** 模块提供公共功能:

```java
// 1. Web 增强
com.molandev.cloud.common.web
  ├── result/        // 统一响应封装
  ├── auth/          // 认证授权
  ├── exception/     // 全局异常处理
  ├── log/           // 操作日志注解
...
// 3. 缓存服务
com.molandev.cloud.common.cache
  ├── CacheService   // 缓存服务接口
  └── RedisCacheServiceImpl  // Redis 实现

// 4. Excel 导入导出
com.molandev.cloud.common.poi
  ├── ExcelUtil      // Excel 工具类
  └── ExcelListener  // 导入监听器
```
...
Gateway 网关
  ...
  │
  ▼
Target Service (System/File/Msg/Task)
  │
  │ 1. Controller 接收请求
  │ 2. 参数验证
  │ 3. 业务处理
