...
```java
@Component
public class NotifyExpiredUserTask {
...
**内置任务示例：**
- `CleanLogicDeleteUser` - 清理逻辑删除用户
- `NotifyExpiredUser` - 用户过期通知
- `CleanDeletedTask` - 清理已删除任务
- `CleanOldTaskLogs` - 清理历史执行日志
- `MoveFileToRecycle` - 临时文件回收
- `CleanRecycle` - 清理回收站文件

尝试删除内置任务时会提示：**内置任务不允许删除**
